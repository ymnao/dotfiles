...
4. **ntfy 設定を生成**
   ...
   ```
   `~/.claude/.ntfy-config` に `server=` / `topic=` の 2 行が保存される
   (chmod 600、git 管理外)。セルフホスト ntfy に向ける場合は環境変数
   `NTFY_SERVER` を設定してから実行する。表示される手順に従ってスマホの
   ntfy アプリで server / topic を購読する。
...
スマホ SSH クライアントによってセットアップ方法が異なる。共通の目的は
「Tailscale ホスト名でつないだ tmux セッションを mosh 経由で保持する」こと。

#### Termius (推奨、公式 Mosh サポートあり)

ローカルシェルを持たない GUI クライアントなのでコマンドを直接打てない。
Host 設定で:

1. Address: Mac の Tailscale ホスト名、Username: Mac のユーザー名
2. Mosh を有効化 (Settings → Host → Use Mosh)
3. Startup Command (or Snippets) に `tmux new -A -s claude`

接続するとそのまま tmux セッションに入る。

#### Blink Shell / Termux など (ローカルシェルを持つクライアント)
...
切断しても tmux セッションが Mac 側で生き続けるため、再接続で続きから
操作できる。
