...
- harness 共通の hook 実装は `agents/hooks/` に正本を置き、`claude/hooks/` と `codex/hooks/` からは相対 symlink で参照する（drift を構造的に防止）。codex 固有 hook（guard-codex-dir / redact-secrets / notify-stop）は `codex/hooks/` に実体のまま置く
...
- `.claude/stop-gate.conf` は Stop hook 検証ゲート（`claude/hooks/stop-verify-gate.sh`）が読むオプトインファイル。conf があるリポジトリのみターン終了前に検証コマンドを実行する
