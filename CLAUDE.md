...
- **feature ブランチに `git merge main` する形も同じ制限で中断する**。main 側が `~/.claude/` へ symlink される実体ファイル（`agents/AGENTS.md` / `claude/rules/` 等）を書き換えていると `error: unable to unlink old ...` に続いて `Merge with strategy ort failed.` で止まる。**この失敗自体は安全**（working tree は無傷、HEAD も動かない。上の `checkout -b` のような半端な状態は残らない）。回避は「main 側の変更内容を **file 編集 tool** で working tree に適用 → commit → 再度 merge」— 差分が消えていれば git はそのファイルを触らないので merge が通る（2026-08-08 に実測。PR #293 merge 後の main を取り込む際に踏んだ）
