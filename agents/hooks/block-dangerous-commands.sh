...
#   `1` が立つ問題も**大半の形では**同時に解消する。**全部ではない**: 区切りが隣接
#   する `cmd 2>&1|2>&1` は 1 個目のマッチが右境界として `|` を消費するため 2 個目に
#   左境界が残らず、裸の `2` が立って readonly 免除が外れる (2026-08-07 実測、exit=2)。
#   bash では両方 fd 複製なので過ブロックだが fail-closed 側であり、main も同じ結果
#   なのでリグレッションではない。
...
# 副作用として 2 種類の FP が増える。どちらも fail-closed 方向で、`.codex` 保護 (#190)
...
#     (2026-08-07 実測: 追加前 exit=0 → 追加後 exit=2)
