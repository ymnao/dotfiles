...
置かない) を選択肢として user に出す**。判断は user のもの。
実例 (2026-09-02 / issue #329): ブランチ名の文字集合を PreToolUse hook で
enforce する案で、round 3 の Critical 2 件 → round 4 の Critical 9 件と増えた。
どれも実測付きの正しい指摘で個別には fix できたが、原因は名前引数の位置を
決めるために bash の word 分割と git の `parse-options` を hook 内で再実装
していたこと。決定打は repo 自身の gitconfig alias (`co` = `checkout`)。
約 250 行と 4 周ぶんのレビューを捨てて撤退した。撤退した側の設計判断は
