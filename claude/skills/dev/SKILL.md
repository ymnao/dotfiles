...
着手前に、このサイクルが **外向き (repo の目的に直接効く: この repo なら
日常的に使う shell・ツール設定、プロダクト repo なら機能・不具合) か
内向き (それを支える基盤: skill / eval / hook / test / CI)** かを 1 行で
宣言する。**外向きが既定**で、内向きは「外向きを進めるために今necessary か」を
1 文で言えるときに選ぶ。

#### 1b. backlog の煙探知 (opt-in・repo ごと・停止しない)
...
- **外向きの変更** (`fish/` `nvim/` `wezterm/` 等の実際の設定、
  `scripts/link.sh` のような配置ロジック): 下記のフル隊列
- **内向きの変更** (`claude/skills/` `codex/skills/` `*/evals/` `tests/`
  `docs/` `.github/` のみを触る): **`/simplify` 1 回のみ**。code-reviewer は
  呼ばない。機械の欠陥は次のセッションで実際に顕在化する自己修正性がある
  一方、機械へのレビューが機械改善 finding を生む正のフィードバックが
  backlog 増加の主因だったため
  - **例外 (内向きでもフル隊列)**: `claude/hooks/` `agents/hooks/`
