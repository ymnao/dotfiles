...
実例 (2026-09-01 / ghirgana #65)。issue は「App Remote の承認が切れ、アプリ内から
取り直す経路が無い」とし、潰した前提の表 (コード回帰・署名・アカウント・web の
OAuth grant・R8) まで揃っていた。それを土台に承認フローを実装し、**レビュー 3 周・
CI 5 回・約 300 行**まで進んだところで、実機のログ 1 本で前提ごと崩れた —
**アプリが Dashboard 未登録の redirect URI を送っていただけ**で、承認は切れて
いなかった。設定 1 行を登録値に差し替えて実機で見る、が最初の 5 分でできていれば、
実装は 1 行も要らなかった。成果物は昇格した学びと、レビューで別途見つかった
バグ修正だけになった。
