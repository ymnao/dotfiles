...
6. **存在理由の点検**: その機能が**なぜ要るのか**を 1 文で書き、その根拠が
   **手元で確かめられているか**を見る。根拠が「以前の調査でそう結論した」
   「外部サービスがそう振る舞うから」の形なら、それは*未検証の外部主張*で
   あり、実装の前に潰す対象。**うち実機でしか確かめられないもの**の扱いは下記
...
##### 前提が実機にしか無いなら、実装の前に前提だけを実機で潰す

**機能の存在理由が「実機でしか観測できない外部の振る舞い」に乗っているとき、
その前提の確認を step 5a (実装後) まで遅らせない。** plan の時点で、
**前提だけを最小の操作で確かめる手順**を user に渡して結果を待つ。

判定は 1 つ: **その前提が崩れたら機能ごと不要になるか**。なるなら先に確かめる。
ならない (前提は実装の細部にしか効かない) なら従来どおり 5a でよい。

- 前提を潰す操作は**設定 1 行の差し替え・ログ 1 本の採取**のような、
  実装を伴わないものを選ぶ。**壊してから見る形になるなら渡さない** (前提が
  生きていた場合に user の環境が壊れたまま残る)。依頼の書き方は 5a と同じ
  — **確認手順と合否の判定基準**を書いて渡す
- 前提が生きていたと確認できてから実装へ進む。崩れたら plan ごと捨てる

実例 (ghirgana #65): 「承認が切れている」前提で承認フローを実装しきった後、実機の
ログ 1 本で前提ごと崩れた (原因は redirect URI の未登録で、設定 1 行で足りた)。

**「issue にそう書いてある」は確かめた根拠にならない。** 前提を作った調査が
どんな条件下で走ったかまで遡れないなら、それは未検証として扱う。

##### 前提を**迂回する** plan を選んだときも、迂回した先を 1 回試す

上は**前提に乗る**設計の話だが、同じ場面で「その前提に乗らない形へ倒す」を選ぶこともある。
そちらは**不確実を避けたのだから安全側**に見えるので、確かめる手が伸びない。だが
**迂回にもコストがあり、それは設計の形に出る** — 動くかどうかでは検出できないので、
実装が終わっても実機が通っても誰も気付かない。

判定は上と同じ 1 つ: **その恐れが存在しなかったら、この plan を選ばなかったか**。
選ばなかったなら、**迂回した先を最小で 1 回動かして確かめてから** plan を確定する
(plan の時点で試せないなら、実機確認の項目に「避けた側の形」を 1 行足す)。

実例 (ghirgana #97): 「画面端で clamp が効かない」恐れを避ける形で PR まで通したが、
避けた形を作ってみると端でもはみ出さなかった。どちらの形も実機では全項目通っていた。
...
**例外: 三択が扱えるのは「まだ載っていないもの」だけ。既に自分が持ち込んだ
ものは、載否の判断より前に、周回数に関わらず即 fix する。** 三択は finding を
...
**直近 2 周で Critical 件数が横ばい以上 (減らない / 増える) なら、finding では
なく設計の深さを疑う。** 三層分類は finding を 1 件ずつ見る仕組みなので、
**個々の指摘が全部正しくて全部 fix 可能なとき**にすり抜け、ループが回り続ける。
見るべきなのは件数の**推移**の方で、減らないのは「実装が悪い」ではなく
「**被覆すべき面積に上限が無い**」のサイン。とくに**外部ツール (shell / git /
外部フォーマットのパーサ) の構文解析を自前で再実装する**形はこれに当たる。
コード上の見分け方は「**対象コマンドの引数位置・トークン分割を独自ロジックで
決めているか**」で、そこが在ると相手の文法 (自分の diff と無関係に広く、
alias / plugin / 設定で user 側が増やせる) を丸ごと追う羽目になり、列挙では
閉じない。このサインが出たら周を重ねず、**撤退 (別の深さで解く / 機構を
置かない) を選択肢として user に出す**。判断は user のもの。ただし
...
#### 5a. 実機確認の要否判断 (テストで観測できない設定を触ったとき)

**テスト・lint・レビューはいずれも設定ファイルの中身しか見ない。**
判定基準は 1 つ: **その変更が正しく効いたことを `make test` で観測できるか**。
できないなら、**必要に応じて** user に実機確認を依頼してから PR に進む。
毎回止める段ではない — 該当するかを 1 回考えるだけでよい。

観測できない典型 (**この列挙は例であって判定基準ではない**。列挙に無い
ツールでも上の基準で判断する):

- 効果が画面・キー入力・起動する子プロセスにしか現れない設定
  (`herdr/` `wezterm/` `fish/` `nvim/` `karabiner/` 等)
- 値が妥当でも**届かないことがある**もの (キーバインド、IME 連携、
  端末依存のエスケープシーケンス)
- 設定の検証コマンド (`<tool> config check` 等) が**構文しか見ない**もの

**挙動が変わらない変更 (コメント・整形・説明文だけ) は対象外**。
`claude/hooks/` `scripts/` `tests/` のように `make test` が挙動そのものを
観測できるものも対象外で、現状どおり PR へ進む。

依頼するときは**確認手順と合否の判定基準**を書く (「開いてみて」ではなく
「新しいペインで `echo $FISH_VERSION` が版番号を返せば OK」の粒度)。
結果は PR の evidence に「実機で確認した項目」として転記し、**未確認の
項目は未確認と書く** (「動くはず」と書かない)。

なお 4-1 のレビュー隊列にも似たディレクトリ列挙があるが、**問いが違うので
統合しない** — あちらは「レビューを厚くすべきか」、ここは「実機で触らないと
分からないか」で、答えが割れる (`claude/hooks/` は前者では最も厚く、後者では
対象外)。共有するのは集合ではなく基準の方 (AGENTS.md「コード品質」節)。

実機確認が要ると分かるのはたいてい step 2 の plan 時点なので、**plan に
「最後に実機確認を挟む」と書いておくとレビュー 2 周が無駄にならない**。
...
昇格する本文はルール (判定基準と手順) を主にし、実例を添えるなら issue / PR
番号と「何が起き、何が原因だったか」の 1-3 行に留める。周回数・行数・調査の
経緯は skill が読まれるたびに払うコストのわりに判断を変えないので、issue と
コミットログに任せる。
...
レビューループの最終 commit 直後にまず `git push origin HEAD` で
push し (ブランチ名をコマンドに代入しない — 理由は `/pr` skill の
step 2)、CI 実行と後続の evidence 組み立て・issue 起票を並走させる
(CI green 確認ゲート自体は維持)。
