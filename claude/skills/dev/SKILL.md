...
**周を追って Critical が減らない・増えるときは、finding ではなく設計の深さを
疑う。** 三層分類は finding を 1 件ずつ見る仕組みなので、**個々の指摘が全部
正しくて全部 fix 可能なとき**にすり抜ける。全部が Tier2 と判定されて全部
fix され、ループは回り続ける。見るべきなのは件数の**推移**の方で、増えるのは
「実装が悪い」ではなく「**被覆すべき面積に上限が無い**」のサイン。とくに
**外部ツール (shell / git / 外部フォーマットのパーサ) の構文解析を自前で
再実装する**形はこれに当たる — 相手の文法は自分の diff と無関係に広く、
user 定義で増える部分 (alias / plugin / 設定) まであると列挙では閉じない。
このサインが出たら周を重ねず、**撤退 (別の深さで解く / 機構を置かない) を
選択肢として user に出す**。判断は user のもの。
...
していたこと。決定打は repo 自身の gitconfig alias (`co` = `checkout`) で、
これは user 定義なので語彙の列挙で閉じない。約 250 行と 4 周ぶんのレビューを
捨てて撤退した。撤退の判断自体は 4 周目まで出てこなかった。
