...
(eval / docs 追記 / skill 追加) にも適用する。Tier3 相当 (予防的・将来の
網羅性向上) は生成しない。eval を書くのは実際に事故が起きた挙動を pin
するときだけ。
...
skill) をこの PR に含める。**事故がなければまるごと skip する** — 毎 PR で
学びを探せば必ず何か出るが、それは学びがあった証拠ではない。同一 PR に
