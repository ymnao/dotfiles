...
実例 (2026-09-10 / ghirgana #97 の PR 3)。固定リングは `Dialog` の別窓なので、その中で
popup を出すと Compose が頼む位置が「窓の中の座標」を「画面の寸法」と比べた結果になる
(javap で確認)。**画面端で clamp が効かない恐れ**を読み取り、それを避ける形を選んで
plan・実装・レビュー 2 周・PR まで通した。実機で user が逆の形を求め、**避けたはずの形を
作ってみたら端でもはみ出さなかった** — 恐れは存在しなかった。どちらの形も実機では全項目
通っており、分かれ目は最後まで「動くかどうか」ではなかった。
...
実例 (2026-09-02 / issue #329): ブランチ名の文字集合を PreToolUse hook で
enforce する案で、Critical が round 1 の 0 件 → round 2 の 2 件 → round 4 の
9 件と増えた (round 3 は fix のみでレビューを回していないので、レビューは
3 周 + fix 1 周)。どれも実測付きの正しい指摘で個別には fix できたが、原因は
名前引数の位置を決めるために bash の word 分割と git の `parse-options` を
hook 内で再実装していたこと。決定打は repo 自身の gitconfig alias
(`co` = `checkout`)。代替は `/issue` 側の機械的受け渡し (検証済みファイルを
`"$(cat ...)"` で読み直す形) で、名前をタイプし直す経路自体が消えるため
同じ脅威を塞ぐ。約 250 行を捨てて撤退した。撤退した側の設計判断は
`claude/skills/issue/SKILL.md` step 9 にある (同じ事故を別の問い —
「なぜこの検証に hook の裏付けを置かないか」— から書いたもの)。
