...
- [ ] round=1 の各 phase が **正確に 1 回ずつ、start → stub-loaded → end
      ...
      ```bash
      awk '/^\[dev\/review-loop\] round=1 phase=(start|stub-loaded|end)/ {
        split($0, a, " "); p=substr(a[3],7)
        gsub(/[^a-z-]/,"",p)
        seq = seq " " p; cnt[p]++
      } END {
        if (seq != " start stub-loaded end") { print "order fail: "seq; exit 1 }
...
- [ ] round=1 で fix / commit / uncommitted change が **発生していない**
      ...
      ```bash
      start_head=$(grep -oE '^\[dev/review-loop\] round=1 phase=start head=[a-f0-9]+ dirty=[01]$' "$transcript" | awk '{print $3" "$4}')
      end_head=$(grep -oE '^\[dev/review-loop\] round=1 phase=end applied=0 status=complete head=[a-f0-9]+ dirty=[01]$' "$transcript" | awk '{print $5" "$6}')
      [ -n "$start_head" ] && [ "$start_head" = "$end_head" ] && \
        echo "$start_head" | grep -q ' dirty=0$'
      ```

      (round=1 の start と end で HEAD SHA が同一かつ両方 dirty=0 なら、
