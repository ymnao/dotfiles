...
- [ ] round=1 で fix / commit / uncommitted change が **発生していない**
      ...
      ```bash
      start_head=$(grep -oE '^\[dev/review-loop\] round=1 phase=start head=[a-f0-9]+ dirty=0$' "$transcript" | grep -oE 'head=[a-f0-9]+')
      end_head=$(grep -oE '^\[dev/review-loop\] round=1 phase=end applied=0 status=complete head=[a-f0-9]+ dirty=0$' "$transcript" | grep -oE 'head=[a-f0-9]+')
      [ -n "$start_head" ] && [ "$start_head" = "$end_head" ]
      ```

      (`dirty=0$` を anchoring regex に折り込むことで両行の dirty=0 は
      grep 成功だけで保証される。start と end で HEAD SHA が同一なら、
