...
- [ ] **★主目的**: round=2 の start 時点から end 時点までに **新たな**
      fix / commit / uncommitted change が加わっていない (REPORT-ONLY
      遵守を機械検証、pre-existing dirty state は許容):
      ...
      end_regex='^\[dev/review-loop\] round=2 phase=end applied=0 status=cap-reached head=[a-f0-9]+ dirty=[01]$'
      ```
