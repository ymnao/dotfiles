...
- [ ] 各周で stub-loaded ログが出て指摘件数が期待どおり:
  ...
  phase 順序検証の awk (06 と同一):

      ```bash
      awk '/^\[dev\/review-loop\] round=([12]) phase=(start|stub-loaded|end)/ {
        split($0, a, " "); r=substr(a[2],7,1); p=substr(a[3],7)
        gsub(/[^a-z-]/,"",p)
        seq[r] = seq[r] " " p; cnt[r"-"p]++
      } END {
        for (r=1; r<=2; r++) {
          if (seq[r] != " start stub-loaded end") { print "order fail r"r": "seq[r]; exit 1 }
          for (p in cnt) if (cnt[p] != 1) { print "dup "p": "cnt[p]; exit 1 }
        }
      }' "$transcript"
      ```
- [ ] **★主目的**: round=2 で fix / commit / uncommitted change が
      **一切発生していない** (REPORT-ONLY 遵守を機械検証):
      ...
      ```bash
      start_head=$(grep -oE '^\[dev/review-loop\] round=2 phase=start head=[a-f0-9]+ dirty=[01]$' "$transcript" | awk '{print $3" "$4}')
      end_head=$(grep -oE '^\[dev/review-loop\] round=2 phase=end applied=0 status=cap-reached head=[a-f0-9]+ dirty=[01]$' "$transcript" | awk '{print $5" "$6}')
      [ -n "$start_head" ] && [ "$start_head" = "$end_head" ] && \
        echo "$start_head" | grep -q ' dirty=0$'
      ```

      (SKILL.md 構造化ログの `head=<sha>` `dirty=<0|1>` を比較。
      round=2 の start と end で HEAD SHA が同一かつ両方 dirty=0 なら、
      Bash / apply_patch / sed 経由も含めて一切の変更がなかったことを
      保証できる。Edit/Write tool call マーカーへの依存を排除)
