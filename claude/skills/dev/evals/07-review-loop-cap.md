...
- [ ] レビューループが **正確に 2 周** 回った:
      `[ "$(grep -c '^\[dev/review-loop\] round=.* phase=start$' "$transcript")" -eq 2 ]`
...
- [ ] **★主目的**: `round=2 phase=start` から `/pr` skill 起動までの区間で
      ...
      call マーカーまたは fix commit ログ行の不在を確認。boolean 形式で
      Pass criteria 独立実行を保つ。grep パターンの `<command-name>`
      `<invoke name="...">` `[git commit]` `* commit <sha>` が実 runner の
      transcript 出力形式に一致するかは要実測、不一致なら追跡 issue に
      委ねる)
