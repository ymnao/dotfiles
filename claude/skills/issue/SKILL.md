...
7. Validate the generated name **before it is substituted into any command**. The issue title is attacker-controlled on a public repo, and git accepts shell metacharacters in ref names (`git check-ref-format --branch 'foo$(id);x'` exits 0), so a name carried straight into a command string gets expanded by the shell. Quoting is not the fix — `"$(...)"` still expands.
    ...
    - Also run `LC_ALL=C tr -d '[:print:]\n' < "$TMPDIR/branch-name.txt" | LC_ALL=C wc -c` and require **0**. The `awk` above cannot see past a NUL byte: measured 2026-09-02, a 12-byte file holding `fix/a\0zzz;x\n` exits **0** because macOS awk (20200816) ends the record at the NUL, so `zzz;x` is never matched against the character class while `$(cat …)` still hands it to git (bash drops the NUL). A validator that reports "safe" about a prefix of what actually gets used is a vacuous pass, not a check
