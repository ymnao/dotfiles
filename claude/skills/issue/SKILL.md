...
7. Validate the generated name **before it is substituted into any command**. The issue title is attacker-controlled on a public repo, and git accepts shell metacharacters in ref names (`git check-ref-format --branch 'foo$(id);x'` exits 0), so a name carried straight into a command string gets expanded by the shell. Quoting is not the fix — `"$(...)"` still expands.
    - Write the generated name to `<scratchpad>/branch-name.txt` using your file-writing tool, **not** a shell redirect — the point is that the name must never appear in a command string (Claude Code: the Write tool; codex: `apply_patch`)
    - `<scratchpad>` is the session scratchpad directory named in the system prompt, written out as a literal path. Do **not** use `$TMPDIR`: it is a per-uid path that carries no session id, so another session running `/issue` concurrently writes the same file, and the name this step validated is not necessarily the name step 9 reads back — that makes the check vacuous, which is the one thing it exists to prevent. `/next` moved `merged-branch.txt` for the same reason; 根拠の正本は `claude/rules/acceptance-patterns.md` の一時ファイル置き場の項
    - Run `LC_ALL=C awk 'NR>1 || $0 !~ /^[abcdefghijklmnopqrstuvwxyz0123456789][abcdefghijklmnopqrstuvwxyz0123456789\/._#-]*$/ {bad=1} END{exit (bad || NR!=1)}' "<scratchpad>/branch-name.txt"`
    - **exit 0** → the name is inside the safe set; steps 8-9 read it back out of the same file
    - **exit 1** → do NOT substitute it anywhere. Run `rm -f "<scratchpad>/branch-name.txt"` first, then report the rejected name and stop. Leaving a rejected name in the hand-off slot is what turns "the check stopped us" back into "the check only binds a cooperative agent": the file stays at the path this session's later runs read from, so a run that resumes at step 8-9 reads the name that *failed* validation. Measured 2026-09-02: `git checkout -b "$(cat …)"` on a file holding `foo$(id);x` creates a branch by that literal name (no shell execution — the substitution output is not re-scanned — but the metacharacters land in a ref name, which is the thing step 7 exists to prevent)
    - Also run `LC_ALL=C tr -d '[:print:]\n' < "<scratchpad>/branch-name.txt" | LC_ALL=C wc -c` and require **0**. Whether the `awk` above sees past a NUL byte depends on the implementation: measured 2026-09-02, a 12-byte file holding `fix/a\0zzz;x\n` exits **0** under macOS awk (20200816), which ends the record at the NUL so `zzz;x` is never matched against the character class, and **1** under ubuntu's awk. `$(cat …)` hands the whole thing to git either way (bash drops the NUL), so on the implementations that pass it the validator reports "safe" about a prefix of what actually gets used — a vacuous pass, not a check. This gate does not depend on which awk you have
...
8. Check if the branch name already exists with `git rev-parse --verify "$(cat "<scratchpad>/branch-name.txt")"`:
...
9. Run `git checkout -b "$(cat "<scratchpad>/branch-name.txt")"` to create the branch
    ...
    - Run steps 7-9 in one pass. If you resume from step 8 or 9 without having written the file in this session, `<scratchpad>/branch-name.txt` is whatever a previous run left there — re-run step 7 rather than reading a stale name. `/next` guards its own hand-off file the same way (`next/SKILL.md` step 3), and deletes it afterwards; do the same here with `rm -f "<scratchpad>/branch-name.txt"` once the branch exists
