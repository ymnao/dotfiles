...
9. Run `git checkout -b "$(cat "$TMPDIR/branch-name.txt")"` to create the branch
    ...
    - `agents/hooks/block-dangerous-commands.sh` enforces the same boundary from the other side: it rejects branch-name arguments outside the safe set, and accepts a whole-token `"$(cat <file>)"` as the escape hatch (issue #329). For the branch-creating forms it covers (`checkout -b/-B/--orphan`, `switch -c/-C/--create/--orphan`, `branch -d/-D/--delete`, `worktree add -b`), a retyped name carrying shell metacharacters is blocked there even if this step is skipped. It does not cover flagless `git branch <name>`
