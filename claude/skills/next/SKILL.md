...
2. **main 更新**: `git checkout main` → `git pull origin main --ff-only`。
   sandbox denyWithinAllow に含まれるパス (`claude/skills/`,
   `.claude/skills/`, `claude/hooks/`, `.claude/hooks/`,
   `claude/settings.json`, `.claude/settings.json`, `.claude/commands/`,
   `.claude/workflows/`, `.mcp.json` 等) に触る PR では unlink 制限で
   checkout / pull / reset --hard が失敗する。状況別 workaround:
   - **feature ブランチ checkout 中**:
     (a) `git diff HEAD origin/main --stat` が空か確認 (squash merge や
         他コミット混入だと非空 → その場合は user Terminal 依頼にフォール
         バック)。
     (b) 空なら `git fetch origin main:main` → `git checkout main` の順
