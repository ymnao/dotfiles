...
2. **main 更新**: `git checkout main` → `git pull origin main --ff-only`。
   ...
   - **feature ブランチ checkout 中** (settings 変更 PR 等で main へ
     切り替えられない場合含む): `git fetch origin main:main` →
   ...
   - **既に main checkout 済みで `git pull` が unlink 失敗**:
     ...
     merge 後 tree は feature branch tree と同一なので working tree
     はそのまま、index/HEAD だけ origin/main に進める
     (自分の未 push commit がある等 working tree が origin/main と
     一致しない場合は使えない)
