...
6. **次タスク候補の提示**: HANDOFF.md の残タスクと open issues
   ...
   **目的側 (日常設定の摩擦) の取りこぼしをここで user に聞かない。**
   摩擦は踏んだ瞬間に user が `/dev <自由文>` で持ち込む経路が既にあり、
   セッション終わりに記憶へ問い合わせる層は重複している。実際 2026-08-09 まで
   毎サイクル聞いて起票は 0 件で、user から「毎回聞かれて面倒」と指摘された。
   **代替として頻度や閾値 (N サイクルに 1 回・外向き比率が下がったら等) を
   置かないこと** — それは `.claude/backlog.conf` が廃止した比率管理の再発。
