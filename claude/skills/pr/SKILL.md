...
2. Check for an existing OPEN PR for this branch (avoids creating a duplicate):
   ...
   - Run `gh pr list --state open --json number,url,headRefName,baseRefName,headRepositoryOwner` as a bare invocation, then read the output and keep the rows whose `headRefName` equals `branch_name`, `baseRefName` equals `base_branch`, and `headRepositoryOwner.login` equals `<owner>`
