...
**実起動禁止の検証**: 06-11 の Pass criteria は codex-review / code-reviewer /
fable サブエージェントの実起動禁止を以下 grep で三重担保する:
...
3 本目は Agent tool の `model:"fable"` 経由のフォールバック起動を検出する
(SKILL.md step 4 「codex 不能時のフォールバック」bullet 由来)。SKILL.md
のフォールバック方式 (model 指定 / subagent_type 指定) が変わったらこの
pattern を追随させる。10-11 の Prompt にはこの literal 形を復唱しない
(自己解説の JSON 転記で transcript に流れると false-fail する)。
...
- **classify-risk tier literal (`scripts/classify-risk.sh` 出力)**:
  `classify-risk.sh` は末尾で `jq -n --arg tier "$tier" ... '{tier: $tier, ...}'`
  を出力し、JSON literal `"tier": "high"` (または `low` / `medium`) が
  ...
  low / medium は末尾 literal のみ差し替えて再利用する。11-A/B/C はこの
  pin を参照する。`classify-risk.sh` の jq 出力形式 (キー名 `tier` /
  値の quote 形式) を変えたらこの pin を同時更新すること。
