...
- `corepack use` / `corepack install` / `corepack prepare` / `corepack enable` も PM の取得・有効化を伴うためブロックされる。`corepack pnpm add` / `corepack yarn add` のようなラッパー経由の委譲呼び出しは、内側の PM を解決して同じ allowlist で判定する
