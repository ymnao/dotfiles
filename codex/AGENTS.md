...
- 許可されるのは `npm ci`、引数なしの `npm install` / `pnpm install` / `yarn install`（ロックファイルからの復元）のみ
- `npm install <package>`、`npm install --save-dev <package>`、`npm exec`、`npx`、`pnpm add`、`pnpm dlx`、`yarn add`、`yarn dlx`、`bun add/install`、`bunx`、`pip install`、`pipx install/inject/run`、`uv add`、`uv tool install/run`、`uvx`、`poetry add` 等はすべてブロックされる
