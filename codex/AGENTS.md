...
- **成果物に書く主張も同じ基準で照合する**。docs やコメントに「実測」「測定日」を添えるときは、その 1 行が対応するツール実行結果を指せるか確認する。**日付を添える運用は、その行が本当に測ったものかを保証しない** — 推論に日付を付けても形式だけは満たせてしまう。特に、否定形の断定(「〜すると壊れる」「〜は効かない」)を設計判断の根拠にするときは、対照付きで測ってから書く
