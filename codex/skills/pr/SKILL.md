...
2. Check for an existing OPEN PR for this branch (avoids creating a duplicate):
   ...
   - **`--head <branch_name>` は使わない**。ref 名には `$(...)` や `;` を含められる (`git check-ref-format --branch 'foo$(id);x'` は exit 0)。この repo は public で `/issue` は issue title からブランチ名を作るため、外部文字列がコマンド文字列に入る経路がある。二重引用符で囲んでも `$(...)` の展開は止まらないので、**名前をコマンドに渡さず出力側で突き合わせる**。同じ理由で `<owner>` の `--jq` 埋め込みもやめている
