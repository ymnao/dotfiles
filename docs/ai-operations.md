...
**この表は強制力を持たない。** `claude/settings.json` にモデルを指定する
フィールドは無く(`effortLevel` のみ)、切り替えは `/model` か CLI 既定に依存する。
`claude/agents/code-reviewer.md` の `model: opus` も **alias 運用が意図**で、
ID 直書きにはしない — alias は上流の世代交代に追随するので表の「Opus 世代」の
内側に収まる。逆に ID を固定すると世代が上がったときレビュアーだけが旧世代に
取り残される(ドリフトの向きが変わるだけ)。frontmatter が完全なモデル ID を
受け付けるかも未検証で、外れたときの壊れ方も分からない。したがって表と実挙動の
一致は、上の実測でしか確かめられない。
...
**是正 trigger**: (a) frontmatter が完全なモデル ID / 別 alias を受け付けると
確認できたら、`code-reviewer` を別系統(Fable 世代)へ寄せることを再検討する、
(b) `codex-review` が恒常的に使えなくなったら、緩和が消えるので即座に見直す。
