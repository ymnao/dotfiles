...
表記規則・同じ glob 挙動を持つかは測っていなかった。** 表記を 1 文字外しても
警告が出ない機構なので、流用すると「設定したのに一度も効いていない」状態が
緑のまま成立する。

測定は保護対象と無関係な `.sbxprobe-*` で行い、**正の対照**として既存 4
エントリと同型の非 glob 絶対パスを 1 本混ぜた(対照が読めてしまったら
「設定が反映されていない」であって glob の可否は結論できない、を切り分けるため)。
判定は exit code ではなく**中身が出力されたか**で行った。

| 測ったこと | 結果 |
|---|---|
| `~/*/**/.sbxprobe-env`(repo 直下 / 1 段ネスト) | 拒否 |
| `~/*/**/.sbxprobe-env.*`(repo 直下 / 2 段ネスト) | 拒否 |
...
- **`denyRead` でも glob は効き、表記規則は denyWrite と同じに見える**
  (`~` 始まり + `*` / `**`)。ただし測ったのは上表の行だけで、
  **`~/**/…` 型と、`**` が 0 段にマッチする `~/<dir>/.env` の形は測っていない**
  (後者は home 直下が allowWrite 外でプローブを置けないため)
...
- **deny 配下は読み取りだけでなく削除・リネーム・名前指定の `ls` も止まる**
  (`rm` / `mv` / `ls <name>` がいずれも `Operation not permitted`)。
  **書き込み(作成・上書き)だけは通る**ので、**agent が deny 配下にファイルを
  作ると自分では消せなくなる**。この測定でも、片付けのたびに設定から
  エントリを外す必要があった。親ディレクトリの `ls -la` と `git status` は
  正常に動く(exit 0)
- **Read / Write などの file 編集 tool は素通りする**(実測)。上表の denyWrite と
  同じ非対称で、**`denyRead` は防御境界ではなく「Bash 経路の読み取りを止める」もの**。
  file 編集 tool 側を塞ぐ hook は今回置いていないので、**秘密情報が agent の
  context に入らないことの根拠にはならない**

**出荷形は `~/*/**/.env` と `~/*/**/.env.local` の 2 本。** 当初案の
`~/*/**/.env.*` は `.env.production` のような別 suffix まで覆えるが、
**`.env.example` / `.env.sample` のような「コミット済みで秘密でないテンプレート」
まで巻き込む**(実測で両方とも読めなくなった)。`cp .env.example .env` の
ような正当な初期設定が Bash 経路で止まるため、実際に秘密が入る 2 名に絞った。
**`.env.production` 等は非カバー**で、これは上の削除不可コストと引き換えに
受容している残余。
