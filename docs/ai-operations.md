...
表記規則・同じ glob 挙動を持つかは測っていなかった。** 表記が効かないときに
警告が出ない性質は denyWrite と共通(上記「denyWrite のパス表記」節)なので、
流用すると「設定したのに一度も効いていない」状態が緑のまま成立する。

測定は保護対象と無関係な `.sbxprobe-*` で行い、**正の対照**として
`/Users/<user>/development/important/dotfiles/.sbxprobe-ctl` という非 glob の
絶対パスを 1 本混ぜた(対照が読めてしまったら「設定が反映されていない」で
あって glob の可否は結論できない、を切り分けるため)。判定は exit code ではなく
**中身が出力されたか**で行った。
...
| `~/*/**/.env.*` を設定した状態の `.env.example` / `.env.sample` | 拒否 |
| 出荷形 `~/*/**/.env` + `~/*/**/.env.local` | `.env` / `.env.local` / `tests/.env.local` を拒否、`.env.example` は読めた |
| deny 配下への `rm` / `mv` / `ls <name>` | 拒否。書き込み(作成・上書き)だけは通る |
| deny 配下を含むディレクトリの `ls -la` / `git status` | 正常(判定はこの 2 行だけ exit code) |
...
sandbox ごと外れる行 / file 編集 tool / この設定自体の改ざん。内訳は上記
「denyWrite のパス表記」節の末尾)。denyRead 固有の追加分は 2 つ:

- **`~/*/` が home 直下 1 階層を必ず消費するので `~/.env` は形の上で非カバー**
- **file 編集 tool が素通りするギャップは、`.codex/` のように二次 hook で
  揃えず、共通の残余として受容する。** `.codex/` は「次に codex を起動した
  ときに host 側でコマンドが走る」実行経路だったのに対し、`.env` の脅威は
  「秘密が agent の context に入る」で、context 汚染は Read tool 以外にも
  grep / エディタ / user の貼り付けなど経路が開いており、file 編集 tool 1 つを
  塞いでも閉じない。したがって **`denyRead` は防御境界ではなく「Bash 経路で
  秘密が transcript に流れるのを止める」もの**で、**秘密が agent の context に
  入らないことの根拠にはならない**
...
経路で止まるため、実際に秘密が入る 2 名に絞った。**`.env.production` 等は
非カバー**で、これは上の削除不可コストと引き換えに受容している残余
(2026-09-06 時点の host 実測: `maxdepth 6` / node_modules 除外で
`.env` + `.env.local` が 5 件、`.env.production` が 1 件)。
