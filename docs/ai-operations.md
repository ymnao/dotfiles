...
**測定のため live 設定を一時変更している間は、並行するサブエージェントを
起動しない。** ここでの測定は `claude/settings.json`(= `~/.claude/settings.json`
への symlink)を書き換えて即時反映させる形なので、**同時に走る別セッションからは
設定が勝手に消えたり戻ったりして見える**。2026-09-06 に実際に起きた: denyRead の
測定中に並行して回していたレビューのサブエージェントが「実効設定から 2 エントリが
一時的に消えた。ファイルは無傷。原因未特定」を finding として返した — 正しい観測
だが、原因はこちらの片付け手順(probe を消すためエントリを外して戻す)だった。
**サブエージェントは自分の変更履歴を持たないので、この形は必ず「原因未特定の
事故」として上がってくる。** 既に起動してしまった場合は、**その観測を事故として
記録する前に自分の変更履歴と突き合わせる** — 実在しない事故を正本に書くと、
次に同じ現象を見た人が存在しない不具合を追うことになる。

**ただし突き合わせは却下の口実ではない。却下してよいのは、消えたエントリと
時刻の両方が自分の変更履歴と一致したときだけ**で、**履歴に無いエントリが 1 つでも
混ざる / 時間帯がずれる場合は事故として記録する**。ここを「だいたい自分のせい」で
丸めると、本物の異常を測定ノイズとして捨てることになり、**実在しない事故を書くより
悪い**(書いた誤りは次のレビューで見つかるが、捨てた観測は二度と出てこない)。
