...
| `/survey`(週次サーベイ)の cron / scheduled task による無人起動 | 設計の本体が候補提示後の user 承認ゲート(adopt / reject / defer)であり、無人起動はその手前の「今週やるか」の判断だけを自動化して、承認待ちを積み上げる。週 1 という頻度に対して常設スケジュールを増やす便益も薄い | 手動起動を 3 週以上連続で忘れ、かつ外向きタスクの供給が枯渇したとき |
