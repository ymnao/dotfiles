...
(実際に 2 世代ぶんずれた。経緯は下記)。具体的な版数は「実測値 + 測定日」の
注記としてのみ書く(`claude/rules/shell.md` の「コメントに書く外部環境の事実は
実測して測定日を添える」と同じ扱い)。注記が古くなっても方針は壊れない。

**この規約を「版数を書くな」と読み違えないこと。** 分けるのは*外れたときの
壊れ方*で、版数を書いてよいのは**外れたら fail-loud になる場所**に限る。
実行コマンドの引数(`claude --model claude-sonnet-5 -p ...` のような eval の
実行手順)は、その ID が失効すればコマンドが落ちるので固定してよい — むしろ
再現性のために固定すべき。ここ §1 が危険なのは*方針の記述*で、外れても
何も落ちず**黙ってずれ続ける**から。
...
サブエージェント(`model: opus`)も Opus 5 に解決された。根拠はどちらも
セッションのシステムプロンプトが報告するモデル名で、alias の解決先を実行基盤の
外から検証したものではない。

- 切り替え: `/model`、Agent ツールの `model` パラメータ
  (例: `Agent(subagent_type: "general-purpose", model: "sonnet", prompt: ...)`
  独立第二意見は `model: "fable"`)
- **`claude/agents/code-reviewer.md` の `model: opus` は alias 運用が意図**。
  モデル ID の直書きにはしない。alias は上流の世代交代に追随するので、この表の
  「Opus 世代」という記述の内側に収まる — 逆に ID を固定すると、世代が上がった
  ときにレビュアーだけが旧世代に取り残される(ドリフトの向きが変わるだけ)。
  agent frontmatter が完全なモデル ID を受け付けるかも未検証で、外れたときの
  壊れ方(起動失敗か、黙って既定にフォールバックか)が分からない
- **メインモデルは repo からは pin できない**。`claude/settings.json` に
  モデルを指定するフィールドは無く(`effortLevel` のみ)、切り替えは `/model` か
  CLI 既定に依存する。だからこの表は「強制」ではなく「方針の記述」であり、
  実挙動との一致は上記の実測で確かめるしかない
...
**alias の指す先が変わったとき**(上流の世代交代、`/model` での恒久的な
切り替え、役割ごとの割り当て変更)に実施する。§1 の表は世代レベルなので
alias の移動そのものは方針違反にならないが、**skill / eval が新しい世代で
壊れていないかは別問題**なので、ここで確かめる:
...
- [ ] **§1 の実測注記を更新する**(モデル名・ID・測定日)。この注記が
      「表と実挙動がずれていないか」を人が判断する唯一の材料なので、
      ここを更新しないと世代レベル記述にした意味が薄れる
